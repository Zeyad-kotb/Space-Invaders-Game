...
std::vector<Alien> Game::CreatAliens()
{
	...
	for (int row = 0; row < 5; row++)
	{
		for (int column = 0; column < 11; column++)
		{
			...
			float x = 75 + column * 55;  //55 * 55 pixels is the alien cellSize//
			float y = 110 + row * 55;
...
void Game::MoveAliens()
{
	for (auto& alien : aliens)
	{
		...
		{
			aliensDirection = 1;
			MoveDownAkiens(4);   // move 4 pixels downwards //
		}
...
void Game::MoveDownAkiens(int distance)
{
	for (auto& alien : aliens)
	{
...
void Game::CheckForCollisions()
{
	...
	// Alien Collision With An Obstcale // 
